...
| `backend/app/core/security.py` | UPDATE | 新增 `generate_invite_code()` + `_base62_encode()` |
| `backend/app/services/auth_service.py` | UPDATE | register() 添加自推荐检查 + 自动生成邀请码 + 邮箱小写化 + IntegrityError 区分 |
| `backend/tests/test_auth_service.py` | UPDATE | 新增 AC5 + AC7 + M1 大小写绕过测试 |
| `backend/tests/test_security.py` | UPDATE | 新增 `generate_invite_code` 直接单元测试 |
