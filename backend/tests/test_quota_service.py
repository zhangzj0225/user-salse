...
class TestGetQuotaInfo:
    ...
    def test_sales_records_with_data(self, db_session):
        """验证 sales_records 返回已支付的下级支付记录"""
        agent = User(email="agent@example.com", role="agent", status="active",
                     account_quota=22, account_used=2)
        child = User(email="child@example.com", role="distributor", status="active")
        db_session.add_all([agent, child])
        db_session.flush()
        child.parent_id = agent.id
        db_session.flush()

        # 创建 2 条已支付 + 1 条 pending（不应出现）
        from datetime import datetime, timezone
        r1 = Payment(user_id=child.id, email="child@example.com", amount=888, status="paid",
                      target_role="member_license", reviewed_at=datetime.now(timezone.utc))
        r2 = Payment(user_id=child.id, email="child@example.com", amount=5000, status="paid",
                      target_role="distributor", reviewed_at=datetime.now(timezone.utc))
        r3 = Payment(user_id=child.id, email="child@example.com", amount=10000, status="pending",
                      target_role="agent")
