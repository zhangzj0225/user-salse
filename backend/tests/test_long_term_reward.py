...
class TestCalculateLongTermReward:
    ...
    def test_agent_gets_5_percent(self, db_session, seed_commission_configs):
        ...
        child1 = _make_user(db_session, "c1@example.com", "agent", parent_id=agent.id)
        child2 = _make_user(db_session, "c2@example.com", "agent", parent_id=agent.id)
    ...
    def test_distributor_gets_4_percent(self, db_session, seed_commission_configs):
        ...
        child1 = _make_user(db_session, "c1@example.com", "distributor", parent_id=dist.id)
        _add_commission(db_session, child1.id, 1000, "t1")
        db_session.commit()
    ...
    def test_agent_excludes_distributor_children(self, db_session, seed_commission_configs):
        ...
        agent = _make_user(db_session, "agent@example.com", "agent")
        agent_child = _make_user(db_session, "agent_c@example.com", "agent", parent_id=agent.id)
        dist_child = _make_user(db_session, "dist@example.com", "distributor", parent_id=agent.id)
        _add_commission(db_session, agent_child.id, 1000, "t1")
        ...
        assert len(records) == 1
        # Only agent_child's 1000 * 0.05 = 50.00 (dist_child excluded)
    ...
    def test_distributor_includes_all_children(self, db_session, seed_commission_configs):
        ...
        agent_child = _make_user(db_session, "agent_c@example.com", "agent", parent_id=dist.id)
        _add_commission(db_session, dist_child.id, 1000, "t1")
        _add_commission(db_session, agent_child.id, 2000, "t2")
        db_session.commit()
    ...
    def test_idempotent(self, db_session, seed_commission_configs):
        ...
        agent = _make_user(db_session, "agent@example.com", "agent")
        child = _make_user(db_session, "c@example.com", "agent", parent_id=agent.id)
